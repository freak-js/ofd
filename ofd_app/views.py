...
def orders(request):
    ...
    for order in orders:
        ...
        order_data.append({'id': order.id, 'order_num': cnt, 'adddate': order.adddate, 'cnt_products': len(rels), 'total': total, 'comment': order.comment, 'products': products, 'status': order.status.code, 'user': order.user})
    filters = {}
